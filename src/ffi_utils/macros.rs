    ...
    ($name:literal, $ptr:expr) => {
        if $ptr.is_null() {
            $crate::ffi_utils::error::set_last_error(
                $crate::ffi_utils::error::FfiError::NullPointer($name.to_string()),
            );
            return 1_i32;
        }
    };
...
macro_rules! ffi_unwrap {
    ($res:expr, $msg:literal) => {
        match $res {
            ...
            Err(e) => {
                $crate::ffi_utils::error::set_last_error(
                    $crate::ffi_utils::error::FfiError::Generic(format!("{}: {}", $msg, e)),
                );
                return 1_i32;
            }
        }
...
macro_rules! ffi_write_bytes {
    ($($name: literal, $bytes: expr, $ptr: ident, $len: ident $(,)?)+) => {
        ...
        $(
            if $ptr.is_null() {
                $crate::ffi_utils::error::set_last_error($crate::ffi_utils::error::FfiError::NullPointer($name.to_string()));
                error_code += 1;
            } else {
                let allocated = *$len;
                *$len = $bytes.len() as c_int;
                if allocated < *$len {
                    $crate::ffi_utils::error::set_last_error($crate::ffi_utils::error::FfiError::Generic(format!(
