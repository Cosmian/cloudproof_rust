...
//unsafe fn rotate_policy(policy: &Policy, attribute: &Attribute) -> Policy {
//let policy_bytes: Vec<u8> = policy.try_into().unwrap();
//let policy_ptr = policy_bytes.as_ptr().cast();
//let policy_len = policy_bytes.len() as c_int;

//let attribute_json = CString::new(attribute.to_string()).unwrap();
...
//let master_public_key_bytes = master_public_key.try_to_bytes().unwrap();
...
//PublicKey::try_from_bytes(&updated_master_public_key_bytes).unwrap();

//(updated_msk, update_master_public_key)
//}
...
//let usk_bytes = usk.try_to_bytes().unwrap();
...
//&msk,
//&AccessPolicy::from_boolean_expression(access_policy).unwrap(),
//&policy,
//)
//.unwrap();
