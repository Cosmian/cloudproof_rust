...
use crate::wasm_bindgen::aesgcm::{webassembly_aesgcm_decrypt, webassembly_aesgcm_encrypt};
...
fn test_encrypt_decrypt() {
    ...
    let ciphertext = webassembly_aesgcm_encrypt(
        plaintext.to_vec(),
        key.to_vec(),
        authentication_data.to_vec(),
    )
    .unwrap();
    let cleartext = webassembly_aesgcm_decrypt(
        ciphertext.to_vec(),
        key.to_vec(),
        authentication_data.to_vec(),
    )
