...
use crate::{log_utils::log_init, Dataset};
...
#[actix_rt::test]
#[serial]
pub async fn test_compact() -> Result<(), FindexRedisError> {
...
#[actix_rt::test]
#[serial]
pub async fn test_upsert_conflict() -> Result<(), FindexRedisError> {
    log_init("cosmian_findex_redis=trace");
    ...
    // the original state is what the user would have fetched before changing the
    // table
    ...
    // the first user is trying to update the table with its new state but knowing
    // the original state
    let rejected = findex
        .upsert_entry_table(UpsertData::new(
