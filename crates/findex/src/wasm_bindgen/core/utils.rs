...
use crate::wasm_bindgen::core::{types::ObjectSourceForErrors, Fetch};
...
macro_rules! callback {
    ($callback_ref:expr, $input:ident) => {{
        ...
        let promise = $crate::wasm_bindgen::core::utils::Promise::resolve(
            &js_function
                .call1(this, &$input)
                .map_err(|e| FindexErr::CallBack(format!("js callback failed {e:?}")))?,
        );
        wasm_bindgen_futures::JsFuture::from(promise)
            .await
            .map_err(|e| FindexErr::CallBack(format!("wait for js callback, {e:?}")))?
    }};
}
...
pub fn js_value_to_encrypted_table<const UID_LENGTH: usize>(
    ...
    for (i, try_obj) in array.values().into_iter().enumerate() {
        let obj = try_obj.map_err(|e| {
            FindexErr::ConversionError(format!("convert encrypted table to bytes array, {e:?}"))
        })?;
...
pub fn get_bytes_from_object_property(
    ...
    let bytes = Reflect::get(obj, &JsValue::from_str(property))
        .map_err(|e| FindexErr::ConversionError(format!("convert to bytes array, {e:?}")))?;
