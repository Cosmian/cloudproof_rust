...
macro_rules! call0 {
    ($obj:ident, $callback:ident) => {{
        if let Some(callback) = &$obj.$callback {
            let this = &$crate::backends::custom::wasm::JsValue::null();
            let js_function = $crate::backends::custom::wasm::Function::from(
                $crate::backends::custom::wasm::JsValue::from(callback),
            );
            let promise =
                $crate::backends::custom::wasm::Promise::resolve(&js_function.call0(this)?);
            $crate::backends::custom::wasm::JsFuture::from(promise).await?
        } else {
            return Err(BackendError::MissingCallback(format!(
...
macro_rules! call1 {
    ($obj:ident, $callback:ident, $input:expr) => {{
        if let Some(callback) = &$obj.$callback {
            ...
            let promise =
                $crate::backends::custom::wasm::Promise::resolve(&js_function.call1(this, $input)?);
            $crate::backends::custom::wasm::JsFuture::from(promise).await?
        } else {
            return Err(BackendError::MissingCallback(format!(
                "No attribute `{}` is defined for `self`",
...
macro_rules! call2 {
    ($obj:ident, $callback:ident, $input1:expr, $input2:expr) => {{
        if let Some(callback) = &$obj.$callback {
            ...
            let promise = $crate::backends::custom::wasm::Promise::resolve(
