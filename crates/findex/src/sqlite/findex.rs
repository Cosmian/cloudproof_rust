...
impl FindexCallbacks<UID_LENGTH> for RusqliteFindex<'_> {
    ...
    async fn fetch_chain_table(
        ...
        while let Some(row) = rows.next().map_err(|e| FindexErr::Other(format!("{e}")))? {
            let uid: Vec<u8> = row.get(0).map_err(|e| FindexErr::Other(format!("{e}")))?;
            chain_table_items.insert(
                Uid::try_from_bytes(&uid)?,
                row.get(1).map_err(|e| FindexErr::Other(format!("{e}")))?,
            );
        }
    ...
    async fn upsert_entry_table(
        ...
        let tx = self
            .connection
            .transaction()
            .map_err(|e| FindexErr::Other(format!("{e}")))?;
        ...
        tx.commit().map_err(|e| FindexErr::Other(format!("{e}")))?;
        Ok(rejected_items)
    }
